...
import {  RegisterApplicationConfig, StandardRegisterApplicationConfig, ActivityFn, Activity, Application,AppLoad, LifeCycleFn, AppChanges } from '../interfaces/applications';
...
export function getAppChanges(apps: Array<Application>): AppChanges {
    ...
    apps.forEach(app => {
        ...
        const appShouldActive = app.status !== "SKIP_BECAUSE_BROKEN" && shouldActive(app);
        switch (app.status) {
    ...
    return {
        appsToLoad, appsToMount, appsToUnmount, appsToUnload
    }
...
function shouldActive(app: Application): Boolean {
    try {
        ...
        handleError(err, app, "SKIP_BECAUSE_BROKEN" );
        return false;
    }
}
...
function sanitizeActiveWhen(activeWhen: Activity): ActivityFn {
    const activenWhenArray: Array<Activity> = Array.isArray(activeWhen) ? activeWhen : [activeWhen];

    const activeWhenFunsArray: Array<ActivityFn> =  activenWhenArray.map((activeWhenItem) => {
        return typeof activeWhenItem === 'function' ? activeWhenItem : pathToActiveWhen(activeWhenItem);
    })
...
function toDynamticPathValidatorRegex(path: string, exactMatch?: boolean): RegExp {
    let lastIndex = 0,
      inDynamic = false,
      regexStr = "^";
